...
export default function EditProduct_Form() {
    const history=useHistory();
    const location =useLocation();
    console.log(location.state.product); //
    const[producttoedit,setProducToEdit]=useState(location.state.product);
  const classes = useStyles();
  const [email, setEmail] = useState(location.state.product.name);
  const [password, setPassword] = useState(location.state.product.description);
  const [mass, setMass] = useState(location.state.product.mass);
...
const uploadProduct = async e => {
  ...
  data.append('id',location.state.product.product_id);
  
  setLoading(true)

  API.editProduct(data).then(res => {
...
<Divider></Divider>
{loading ? (
        <img src={location.state.product.sprite} style={{ width: '200px',height:'200px' }}/>
      ) : (
        <img src={location.state.product.sprite} style={{ width: '200px',height:'200px' }} />
      )}
